...
from jarvis.arm.src.utils.vpt_lib import misc
from jarvis.arm.src.utils.vpt_lib import torch_util as tu
...
class FiLMConditioning(nn.Module):
    ...
    def __init__(self, cond_channels: int, feat_channels: int) -> None:
        super().__init__()
        self._projection_add = nn.Linear(cond_channels, feat_channels)
        self._projection_mul = nn.Linear(cond_channels, feat_channels)
        # nn.init.zeros_(self._projection_add.weight)
        # nn.init.zeros_(self._projection_add.bias)
        # nn.init.zeros_(self._projection_mul.weight)
        # nn.init.zeros_(self._projection_mul.bias)
        
    def forward(self, feat: torch.Tensor, cond: torch.Tensor) -> torch.Tensor:
        projected_add = self._projection_add(cond)
        projected_mul = self._projection_mul(cond)
        if feat.ndim == 4:
            projected_add = projected_add.reshape(feat.shape[0], feat.shape[1], 1, 1)
            projected_mul = projected_mul.reshape(feat.shape[0], feat.shape[1], 1, 1)
        return feat * (1 + projected_mul) + projected_add
...
class CnnBasicBlock(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        flag = kwargs.get('require_goal_embedding', False)
        if flag and flag is not None:
            print("Setup FiLM conditioning layer for Impala CNN")
            self.film_conditioning_layer = FiLMConditioning(cond_channels=512, feat_channels=self.inchan)

    def forward(self, x, cond=None):
        x = x + self.conv1(self.conv0(x))
        if cond is not None:
            x = self.film_conditioning_layer(x, cond)
        return x 
    ...
    def forward(self, x, cond=None):
        ...
        if self.pool:
            ...
            if self.post_pool_groups is not None:
                x = self.n(x)
        x = tu.sequential(self.blocks, x, cond=cond, diag_name=self.name)
        return x
...
class ImpalaCNN(nn.Module):
    ...
    def forward(self, x, cond=None):
        ...
        x = tu.sequential(self.stacks, x, cond=cond, diag_name=self.name)
