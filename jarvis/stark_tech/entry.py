...
class Minecraft(gym.Env):
    ...
    """
    def __init__(self, frameskip = 1, resolution = [640,360],
    ...
    def reset(self):
        ...
        if len(self.enable_tasks) > 0:
            ...
            self.current_task_conf = self.task_conf[task_key]
        ...
        info = {} if len(self.current_task_conf) == 0 else {'text': self.current_task_conf['text']}
...
def env_generator(env_conf: Dict, **kwargs) -> "MineRL Environment":
    ...
    if hasattr(env_conf, "task_conf") and env_conf.task_conf is not None:
        task_conf = dict()
        for task, conf in env_conf.task_conf.items():
            task_conf[task] = dict(
                text=conf['text'], 
                reward_generator=RewardGenerator(reward_conf=conf['reward_conf'])
            )
    else:
        task_conf = dict()
    
    if hasattr(env_conf, 'enable_tasks') and env_conf.enable_tasks is not None:
        enable_tasks = env_conf.enable_tasks
        for task in enable_tasks:
            assert task in task_conf, f"Task {task} is not defined in task_conf!"
    else:
        enable_tasks = []
    ...
    generate_conf = dict(
        fast_reset=env_conf.fast_reset, 
